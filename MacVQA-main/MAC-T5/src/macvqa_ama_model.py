...
class JointEncoder(T5Stack):
    ...
    def forward(
        ...
        # head_mask = self.get_head_mask(head_mask, self.config.num_layers)这里
...
class VLT5(T5ForConditionalGeneration):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # 原型特征提取
        Q_proto = retrievaled_Q_proto.detach()  # [bs, 1, d_model]
        V_proto = retrievaled_V_proto.detach()  # [bs, 1, d_model]

        # 门控计算
        pooled_hidden = torch.mean(hidden_states, dim=1)  # [bs, d_model]
        gate_input = torch.cat([pooled_hidden, 
                               Q_proto.squeeze(1), 
                               V_proto.squeeze(1)], dim=1)  # [bs, 3*d_model]
        gate = torch.sigmoid(self.gate_layer(gate_input))  # [bs, 2]
        gate_Q = gate[:, 0].unsqueeze(-1).unsqueeze(-1)  # [bs, 1, 1]
        gate_V = gate[:, 1].unsqueeze(-1).unsqueeze(-1)  # [bs, 1, 1]

        # 动态特征融合
        Q_proto_expanded = Q_proto.expand_as(hidden_states)  # [bs, L+V_L, d_model]
