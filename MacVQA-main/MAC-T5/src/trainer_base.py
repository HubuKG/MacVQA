...
class TrainerBase(object):
    ...
    def create_config(self):
        ...
        # 确保本地路径指向 t5-base 文件夹
        backbone_path = "/MAC-T5/models/t5-base"

        print(f"Backbone path: {backbone_path}")  # 打印路径，检查是否正确

        # 确保本地路径存在且包含 config.json 文件
        if os.path.exists(backbone_path) and os.path.isfile(os.path.join(backbone_path, 'config.json')):
    # 只从本地加载配置文件
            config = AutoConfig.from_pretrained(backbone_path, local_files_only=True)
        else:
            raise ValueError(f"Backbone path {backbone_path} not found or config.json is missing.")

        config.feat_dim = self.args.feat_dim
        config.pos_dim = self.args.pos_dim
        config.n_images = 2

        config.use_vis_order_embedding = self.args.use_vis_order_embedding
        ...
        config.use_vis_layer_norm = self.args.use_vis_layer_norm
        config.individual_vis_layer_norm = self.args.individual_vis_layer_norm
        config.losses = self.args.losses

        config.share_vis_lang_layer_norm = self.args.share_vis_lang_layer_norm
        config.classifier = self.args.classifier
...
# 指定本地模型路径
    ...
    # 设置 tokenizer_name 为本地路径
        tokenizer_name = "/MAC-T5/models/t5-base"

    # 打印路径，检查是否正确
        print(f"Tokenizer path: {tokenizer_name}")

    # 确保本地路径存在
        if not os.path.exists(tokenizer_name):
            raise ValueError(f"Tokenizer path {tokenizer_name} does not exist.")
