...
class VQAFineTuneDataset(Dataset):
    def __init__(self, coco_Ours, Examplar_set, split='train', raw_dataset=None, rank=-1, topk=-1, verbose=True, args=None, mode='train', task='q_what', cates=[0,1,2]):
        ...
        self.source_to_h5 = {
            'train': coco_feature_dir.joinpath(f'train2014_enhanced.h5'),
            'minival': coco_feature_dir.joinpath(f'val2014_enhanced.h5'),
            'nominival': coco_feature_dir.joinpath(f'val2014_enhanced.h5'),
            'test': coco_feature_dir.joinpath(f'test2015_enhanced.h5'),

            'vg': dataset_dir.joinpath('VG/features').joinpath('vg_gqa_obj36.h5'),

            'train2014': coco_feature_dir.joinpath(f'train2014_enhanced.h5'),
            'val2014': coco_feature_dir.joinpath(f'val2014_enhanced.h5'),
        }
    ...
    def __getitem__(self, idx):
        ...
        if self.args.use_vision:
            ...
            feats = f[f'{img_id}/features'][()]  # 直接获取数据
            feats = feats.squeeze(axis=1) if feats.ndim == 3 else feats  # 自动压缩多余维度
            assert feats.shape == (self.n_boxes, 2048), f"特征形状错误：{feats.shape}，应({self.n_boxes}, 2048)"
